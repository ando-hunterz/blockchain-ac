...
const border = (index) => {
  ...
  if(index == state.page.length -1) return {"rounded-r-md": true, "border-l": true}
...
<template>
  <nav-bar>
    ...
    <div class="bg-white p-4 mt-4 rounded-md drop-shadow">
      ...
      <div
        v-for="(user, index) in state.users.slice(state.index, state.index + 10)"
        :key="index"
      ...
      <div class="flex flex-row">
        ...
        <button v-if="!user.disabled" @click="disableUser(user.address, index)" class="md:w-8">
          🔐
        </button>
        <button v-else @click="enableUser(user.address, index)" class="md:w-8">🔓</button>
      </div>
    ...
    <div class="flex flex-row mt-2">
      <div v-for="(page, index) in state.page" :key="index">
        <button :class="border(index)"  class="bg-blue-400 px-2 text-white" @click="changePage(index)">{{ page }}</button>
      </div>
    </div>
  </nav-bar>
</template>
